...
export function MobileActionBar() {
  ...
  const anchor = locations.find((l) => l.isOriginal) ?? locations[0];
  const matched = locations.find((l) => pathname?.startsWith(`/locations/${l.slug}`));
  const active = matched ?? anchor;
  if (!active) return null;

  const mapsHref = `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(active.mapsQuery)}`;

  return (
    <div className="fixed inset-x-0 bottom-0 z-40 flex border-t border-ink/15 bg-ink text-plaster md:hidden">
      <a
